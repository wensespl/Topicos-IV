...
def generalized_reduced_gradient():
    ...
    fx = x1**2 + x2**2 + x3**2 + x4**2 - 2*x1 - 3*x4			# Function to be minimized
    hxs = [2*x1 + x2 + x3 + 4*x4 - 7, x1 + x2 +
           2*x3 + x4 - 6]			# Constraints to be obeyed
    ...
    xcurr = np.array([2, 2, 1, 0])							# Starting solution
    ...
    for outer_iter in range(max_outer_iter):
        ...
        print('\n\nOuter loop iteration: {0}, optimal solution: {1}'.format(
            outer_iter + 1, xcurr))
        ...
        delta_h = np.array([[dh.subs(zip(xvars, xcurr)) for dh in dhx]
                           for dhx in dhxs])		# Value of h'_i(xcurr) for all i
        # Computation of J and C matrices
        ...
        while alpha > 0.001:
            ...
            for iter in range(max_iter):
                print('Iteration: {0}, optimal solution obtained at x = {1}'.format(
                    iter + 1, v))
                ...
                delta_h_v = np.array([[dh.subs(zip(xvars, v))
                                     for dh in dhx] for dhx in dhxs])
                J_inv_v = np.linalg.inv(
                    np.array([dhx[nonbasic_vars:] for dhx in delta_h_v], dtype=float))
